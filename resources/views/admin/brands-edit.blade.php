...
@section('content')
    ...
    <div class="main-content-inner">
        <div class="main-content-wrap">
            ...
            <!-- new-category -->
            <div class="wg-box">
                <form class="form-new-product form-style-1 needs-validation" action="{{ route('admin.brands.update') }}"
                    method="POST" enctype="multipart/form-data" novalidate>
                    @csrf
                    <input type="hidden" hidden name="id" value="{{ $brand->id }}" />
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('name') is-invalid @enderror" type="text" placeholder="Brand name"
                            name="name" tabindex="0" aria-required="true" value="{{ $brand->name }}" required
                    ...
                    <fieldset class="name">
                        ...
                        <input class="flex-grow @error('slug') is-invalid @enderror" type="text" placeholder="Brand Slug"
                            name="slug" tabindex="0" value="{{ $brand->slug }}" aria-required="true" required
                    ...
                    <fieldset>
                        <div class="body-title">{{ __('Upload images') }} <span class="tf-color-1">*</span>
                        </div>
                        <div class="upload-image flex-grow">
                            @if ($brand->image)
                            <div class="item" id="imgpreview" >
                                <img src="{{ asset('storage/images/brands/'.$brand->image)}}" class="effect8" alt="">
                            </div>
                            @endif
                            ...
                            <div id="upload-file" class="item up-load">
                                <label class="uploadfile" for="myFile">
                                    ...
                                    <span class="body-text">{{ __('Drop your images here or select') }} <span
                                            class="tf-color">{{ _('click to browse') }}</span></span>
